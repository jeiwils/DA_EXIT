...
from DA_EXIT.preprocessing_ingestion.a1_ingestion.chunking import (
    split_long_passage,
    split_long_passage_discourse_aware_with_flags,
)
from DA_EXIT.preprocessing_ingestion.utils.algorithms.discourse_aware import (
    _expand_sentence_text,
    _has_anaphora_marker,
    _has_cataphora_marker,
    iter_sentence_spans,
)
from DA_EXIT.preprocessing_ingestion.a1_ingestion.dataset_preprocessing_functions import (
    DEFAULT_OUTPUTS,
    process_dataset,
    process_acord_split,
    sentence_passages_from_full,
)
from DA_EXIT.preprocessing_ingestion.a2_indexing.splits import carve_validation_split
...
def _build_fever_passages(*, shared_path: Path, resume: bool) -> None:
    ...
    wiki_dir = data_path("raw_datasets", "FEVER", "wiki-pages", "wiki-pages")
...
def _ensure_fever_passages(
    *,
    ...
    shared_path = data_path("processed_datasets", "fever", "wiki_passages.jsonl")
...
def main() -> None:
    ...
    for dataset in tqdm(DATASETS_TO_PREPROCESS, desc="datasets"):
        for split in tqdm(SPLITS, desc=f"{dataset}/splits", leave=False):
            ...
            if dataset == "acord":
                raw_dir = data_path("raw_datasets", "ACORD")
            ...
            if dataset == "fever":
                split_files = {
                    "train": str(data_path("raw_datasets", "FEVER", "train.jsonl")),
                    "dev": str(
                        data_path("raw_datasets", "FEVER", "shared_task_dev.jsonl")
                    ),
                    "test": str(
                        data_path("raw_datasets", "FEVER", "shared_task_test.jsonl")
                    ),
                }
                file_path = split_files.get(split)
                if file_path is None:
            ...
            if dataset == "hotpotqa":
                if split == "train":
                    file_path = str(
                        data_path("raw_datasets", "hotpotqa", "hotpot_train_v1.1.json")
                    )
                elif split == "dev":
                    file_path = str(
                        data_path(
                            "raw_datasets", "hotpotqa", "hotpot_dev_fullwiki_v1.json"
                        )
                    )
                elif split == "test":
                    file_path = str(
                        data_path(
                            "raw_datasets", "hotpotqa", "hotpot_test_fullwiki_v1.json"
                        )
                    )
                else:
                ...
                file_path = str(
                    data_path("raw_datasets", "2wikimultihopqa", f"{split}.json")
                )
                field_map = {
                    "get_id": lambda ex: ex["_id"],
                    "get_question": lambda ex: ex["question"],
                ...
                file_path = str(
                    data_path(
                        "raw_datasets", "musique", f"musique_ans_v1.0_{split}.jsonl"
                    )
                )
                field_map = {
