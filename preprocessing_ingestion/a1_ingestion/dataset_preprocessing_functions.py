...
To process a HotpotQA style JSON file::
    ...
    from DA_EXIT.preprocessing_ingestion.a1_ingestion.dataset_preprocessing_functions import process_dataset
    from DA_EXIT.preprocessing_ingestion.utils.__utils__ import pid_plus_title, data_path
    ...
    process_dataset(
        ...
        split="dev",
        file_path=str(
            data_path("raw_datasets", "hotpotqa", "hotpot_dev_fullwiki_v1.json")
        ),
        field_map=field_map,
    )
...
def iter_acord_training_packages(
    *,
    split: str,
    raw_dir: str | Path = data_path("raw_datasets", "ACORD"),
...
DATASET_CONFIGS: Dict[str, Dict[str, Any]] = {
    "hotpotqa": {
        "file_path": lambda split: (
            str(data_path("raw_datasets", "hotpotqa", "hotpot_train_v1.1.json"))
            if split == "train"
            else str(
                data_path("raw_datasets", "hotpotqa", "hotpot_dev_fullwiki_v1.json")
            )
    ...
    "musique": {
        "file_path": lambda split: str(
            data_path("raw_datasets", "musique", f"musique_ans_v1.0_{split}.jsonl")
        ),
        "field_map": {
...
"""Dataset-specific ingestion helpers for DA_EXIT scripts."""
