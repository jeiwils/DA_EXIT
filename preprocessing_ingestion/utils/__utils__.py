...
def _find_da_exit_root(start: Path) -> Path:
    """Walk up from ``start`` to locate the DA_EXIT repo root."""
    for parent in (start, *start.parents):
        if parent.name == "DA_EXIT":
            return parent
    return start.parent


DA_EXIT_ROOT = _find_da_exit_root(Path(__file__).resolve())
DATA_DIR = DA_EXIT_ROOT / "data"


def data_path(*parts: str) -> Path:
    """Return a path under the DA_EXIT data directory."""
...
def resolve_root(model: str, dataset: str, split: str, variant: str) -> Optional[str]:
    ...
    for hoprag_version in FOLDERS_BY_VARIANT[variant]:
        root = data_path("models", model, dataset, split, hoprag_version)
...
def get_result_paths(model, dataset, split, variant):
    base = data_path("results", model, dataset, split, variant)
    return {
        "base": base,
...
def processed_dataset_paths(dataset: str, split: str) -> Dict[str, Path]:
    ...
    Creates ``DA_EXIT/data/processed_datasets/{dataset}/{split}/`` if necessary and
    ...
    """
    base = data_path("processed_datasets", dataset, split)
...
def model_shard_dir(model: str, dataset: str, split: str) -> Path:
    ...
    The directory follows ``DA_EXIT/data/models/{model}/{dataset}/{split}/shards`` and
    is created if it does not already exist.
    """
    base = data_path("models", model, dataset, split, "shards")
    base.mkdir(parents=True, exist_ok=True)
    return base
