...
from DA_EXIT.preprocessing_ingestion.utils.dataset_utils import build_training_examples, load_dataset_split
from DA_EXIT.preprocessing_ingestion.utils.__utils__ import (
    data_path,
    processed_dataset_paths,
)
from DA_EXIT.LoRa.utils.lora_helpers import (
...
from DA_EXIT.LoRa.utils.supervised_learning_utils import grid_search
from DA_EXIT.LoRa.tests.plot_confusion_matrix import plot_confusion
...
DEFAULT_OUTPUT_DIR = str(data_path("models", "useful_sentence_lora"))
DEFAULT_OUTPUT_DIR_DISCOURSE_AWARE = str(
    data_path("models", "useful_sentence_lora_discourse_aware")
)
DEFAULT_MAX_LEN = 256
...
if __name__ == "__main__":
    ...
    if USE_DISCOURSE_AWARE_PASSAGES:
        train_kwargs.update(
            {
                "train_passages_path": str(
                    data_path(
                        "processed_datasets",
                        DEFAULT_DATASET,
                        DEFAULT_TRAIN_SPLIT,
                        "passages_discourse_aware.jsonl",
                    )
                ),
                "dev_passages_path": str(
                    data_path(
                        "processed_datasets",
                        DEFAULT_DATASET,
                        DEFAULT_DEV_SPLIT,
