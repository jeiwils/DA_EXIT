...
from DA_EXIT.sentence_extraction_reranking.utils.x_config import LLM_DEFAULTS, MAX_TOKENS, TEMPERATURE
...
def _wrap_for_deepseek_user(prompt: str, task: str, reason: bool = True) -> str:
    ...
    if reason and task in {"answer_generation"}:
        preface = (
...
def query_llm(
    ...
    max_tokens:
        ...
        provided, the value is pulled from :data:`DA_EXIT.sentence_extraction_reranking.utils.x_config.MAX_TOKENS`.
        Defaults to ``128`` when no phase is available.
    temperature:
        Sampling temperature. If ``None`` and ``phase`` is provided, the value
        is pulled from :data:`DA_EXIT.sentence_extraction_reranking.utils.x_config.TEMPERATURE`.
        Defaults to ``0.2`` when no phase is available.
    """
